...
class Decoder_L2R(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.cata_embedding = nn.Linear(params['Kcata'], params['m'])
        self.relation_embedding = nn.Embedding(params['Kre'], params['re_m'])

        self.gru00 = nn.GRUCell(params['m']+params['re_m']+params['m'], params['n'])
        ...
        self.cata_probility = CataProbility(params)
        ...
        self.param_Kcata = params['Kcata']
        self.param_Kre = params['Kre']

    def forward(self, ctx_val, ctx_mask, C_ys, C_catas, P_ys, P_catas, P_y_masks, 
        ...
        predict_catas = torch.zeros(length, B, self.param_Kcata).cuda()
        ...
        for i in range(length):
            predict_childs[i], ht, attention, attention_hidden, ct = self.get_child(ctx_val, ctx_key_object, ctx_mask, 
                attention_past, hidden_attention_past_ch, P_ys[i], P_catas[i], P_y_masks[i], P_res[i], ht, R2L_hidden_ch, R2L_hidden_key_ch, R2L_hidden_mask) # 增加了输入R2L的V与K
            ...
            predict_relations[i], ht_relation, hidden_attention_re = self.get_relation(ctx_val, ctx_key_relation, 
                ctx_mask, C_ys[i], C_catas[i], ht_relation, ct, R2L_hidden_re, R2L_hidden_key_re, R2L_hidden_mask, hidden_attention_past_re) # 增加了输入R2L的V与K
    ...
    def get_child(self, ctx_val, ctx_key, ctx_mask, attention_past, hidden_attention_past, p_y, p_y_cata, p_y_mask, p_re, ht, R2L_hidden_ch, R2L_hidden_key_ch, R2L_hidden_mask):
        # print(p_y)
        # print(p_y.dtype)
        p_y = self.object_embedding(p_y)
        p_re = self.relation_embedding(p_re)
        p_cata = self.cata_embedding(p_y_cata)
        p = torch.cat([p_y, p_re, p_cata], dim=1)
        ...
        predict_cata = self.cata_probility(ct, ht, p_y, p_cata, p_re, ct_hidden)

        return predict_child, predict_cata, ht, attention, attention_hidden.permute(1,0), ct
    
    def get_relation(self, ctx_val, ctx_key, ctx_mask, c_y, c_cata, ht, ct, R2L_hidden_re, R2L_hidden_key_re, R2L_hidden_mask, hidden_attention_past_re):
        c_y = self.object_embedding(c_y)
        c_cata = self.cata_embedding(c_cata)
        ...
        predict_relation = self.relation_probility(ct, c_y, c_cata, ht, ct_hidden)
...
class CataProbility(nn.Module):
    def __init__(self, params):
        super(CataProbility, self).__init__()
        self.fc_ct = nn.Linear(params['D'], params['m'])
        self.fc_ht = nn.Linear(params['n'], params['m'])
        self.fc_ctht = nn.Linear(params['n'], params['m'])
        self.fc_p_y = nn.Linear(params['m'], params['m'])
        self.fc_p_cata = nn.Linear(params['m'], params['m'])
        self.fc_p_re = nn.Linear(params['re_m'], params['m'])
        self.dropout = nn.Dropout(p=0.2)
        self.fc_probility = nn.Linear(int(params['m'] / 2), params['Kcata'])
        

    def forward(self, ct, ht, p_y, p_cata, p_re, ct_h):
        clogit = self.fc_ct(ct) + self.fc_ht(ht) + self.fc_p_y(p_y) + self.fc_p_re(p_re) + self.fc_p_cata(p_cata) + self.fc_ctht(ct_h)
        # maxout 
...
class RelationProbility(nn.Module):
    def __init__(self, params):
        ...
        self.fc_c_cata = nn.Linear(params['m'], params['mre'])
        self.fc_ht = nn.Linear(params['n'], params['m'])
        self.fc_ctht = nn.Linear(params['n'], params['m'])
        self.dropout = nn.Dropout(p=0.2)
    ...
    def forward(self, ct, c_y, c_cata, ht, ct_h):
        re_probility = self.fc_ct(ct) + self.fc_c_y(c_y) + self.fc_ht(ht) + self.fc_ctht(ct_h) + self.fc_c_cata(c_cata)
