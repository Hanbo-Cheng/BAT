...
class RelationProbility(nn.Module):
    def __init__(self, params):
        ...
        self.fc_adaptive = nn.Linear(params['mre'], params['Kre'])
        self.dropout2 = nn.Dropout(p=0.2)
    

    def forward(self, ct, c_y, ht):
        re_probility = self.fc_ct(ct) + self.fc_c_y(c_y) + self.fc_ht(ht)
        threshold = self.fc_adaptive(re_probility)
        threshold = self.dropout2(threshold)
        ...
        re_probility = re_probility + threshold
