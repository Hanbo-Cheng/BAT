...
class Encoder_Decoder_Bi_Asyn(nn.Module):
    def __init__(self, params):
        ...
        self.init_context_1 = nn.Linear(params['D'], params['n'])
        self.init_context_2 = nn.Linear(params['D'], params['n'])
    ...
    def forward(self, params, x, x_mask, C_y,  
        ...
        init_state_1 = torch.tanh(self.init_context_1(ctx_mean))  # (batch,n)
        init_state_2 = torch.tanh(self.init_context_2(ctx_mean))  # (batch,n)

        #decoder

        C_y_L2R, C_y_R2L = C_y
        P_y_L2R, P_y_R2L = P_y
        P_re_L2R, P_re_L2R = P_re
        ...
        predict_objects_R2L, predict_relations_R2L, predict_objects_pix_R2L, R2L_hidden_states_ch, R2L_hidden_states_re = self.decoder_R2L(ctx, ctx_mask, 
                C_y_R2L, P_y_R2L, y_mask_R2L, P_re_L2R, init_state_1, length)
        predict_objects_L2R, predict_relations_L2R, predict_objects_pix_L2R, attention_hidden_ch = self.decoder_L2R(ctx, ctx_mask, 
                C_y_L2R, P_y_L2R, y_mask_L2R, P_re_L2R, init_state_2, length, R2L_hidden_states_ch, R2L_hidden_states_re,y_mask_R2L)
    ...
    def greedy_inference(self, x, x_mask, max_length, p_y, p_re, p_mask):
        ...
        R2L_hidden_states_ch = torch.zeros(max_length, B, init_state_1.shape[-1]).cuda()
        R2L_hidden_states_re = torch.zeros(max_length, B, init_state_1.shape[-1]).cuda()


        ht = init_state_1
        ht_relation = init_state_1
