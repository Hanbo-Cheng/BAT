...
class Decoder_R2L(nn.Module):
    def __init__(self, params):
        super(Decoder_R2L, self).__init__()
    ...
    def forward(self, ctx_val, ctx_mask, C_ys, P_ys, P_y_masks, 
        ...
        hidden_states_ch = torch.zeros(length, B, init_state.shape[-1]).cuda()
        hidden_states_re = torch.zeros(length, B, init_state.shape[-1]).cuda()
        ...
        for i in range(length):
            predict_childs[i], predict_childs_lm[i], ht, ht_lm, attention, ct = self.get_child(ctx_val, ctx_key_object, ctx_mask, 
                attention_past, P_ys[i], P_y_masks[i], P_res[i], ht, ht_lm)
            hidden_states_ch[i] = ht  # catch the ith ht
            ...
            hidden_states_re[i] = ht_relation

        return predict_childs, predict_childs_lm, predict_relations, predict_childs_pix, hidden_states_ch, hidden_states_re
    ...
    def get_child(self, ctx_val, ctx_key, ctx_mask, attention_past, p_y, p_y_mask, p_re, ht, ht_lm):
        # print(p_y)
        # print(p_y.dtype)
        ...
        ht_hat_lm = self.gru00(p, ht_lm)  #att_query
        ...
        predict_child_lm = self.object_probility(ct, ht_lm, p_y, p_re)

        return predict_child, predict_child_lm, ht, ht_lm, attention, ct
