...
from .frac_bilinear_pooling import *
...
class ObjectProbility(nn.Module):
    def __init__(self, params):
        ...
        self.fuse = FactorizedBilinearPooling(dim=params['n'])
    ...
    def forward(self, ct, ht, p_y, p_re, ct_h):
        clogit = self.fc_ct(ct) + self.fc_p_y(p_y) + self.fc_p_re(p_re) + self.fuse(ht,ct_h)
        # maxout 
...
class RelationProbility(nn.Module):
    ...
    def forward(self, ct, c_y, ht, ct_h):
        re_probility = self.fc_ct(ct) + self.fc_c_y(c_y) + self.fuse(ht, ct_h)
